...
impl MemoryBankController for MBC3 {
    fn read_byte_rom(&self, rom: &Box<[u8]>, rom_size: usize, address: u16) -> u8 {
		if address < 0x4000 {
			...
			if bank_offset + address as usize >= rom_size {
				0xFF
			}
			else {
				rom[bank_offset + (address - 0x4000) as usize]
			}
		}
    }
	...
	fn read_byte_ram(&self, ram: &Box<[u8]>, ram_size: usize, address: u16) -> u8 {
        if self.ram_timer_enable {
			match self.ram_bank {
				0...3 => {
					let bank_offset: usize = self.ram_bank as usize * RAM_BANK_SIZE;
					if bank_offset + address as usize >= ram_size {
						0xFF
					}
					else {
						ram[bank_offset + address as usize]
					}
				},
				0x8...0xC => {
